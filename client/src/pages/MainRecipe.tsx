...
export const MainRecipe = () => {
  ...
  useEffect(() => {
    if (!dataRecipes.recipes || dataRecipes.recipes.length === 0) {
      dispatch(getRecipes());
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 w-[98%] flex flex-col justify-center mx-auto my-[1%]">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        {loading ? (
          <>
            <Skeleton.Input active size="large" className="md:w-1/2 w-full" />
            <Skeleton.Input active size="large" className="md:w-1/4 w-full" />
            <Skeleton.Input active size="large" className="md:w-1/4 w-full" />
          </>
        ) : (
